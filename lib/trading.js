...
exports.findTrading = function(callback) {
    ...
    sync.fiber(function() {
        ...
        var time = today.format('HH') + (Math.floor(minute / 10) * 10).padLeft(2,0);
        var stocklist = [];
        ...
        for(var i=0;i < stocklist.length; i++)
        {
            ...
            */
            ...
            //매도량 기준으로 내림 차순 정리
            var memberlist = result.block1;
            memberlist.sort(function(a, b) {
                var v1 = a.askvol.replace(/,/gi, '');
                v1 = parseFloat(v1);
                var v2 = b.askvol.replace(/,/gi, '');
                v2 = parseFloat(v2);
                return v2 - v1;
            });
            ...
            for(var j=0; j < memberlist.length; j++)
            {
                ...
                if(isInMemberList(memberlist, member.mbr_nm)) {
                    ...
                    netaskvolSum += number;
                    console.log(number, netaskvolSum);
