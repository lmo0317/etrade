...
function getRoundTime(date) {
    return date.format('HH') + (Math.floor(minute / perMinute) * perMinute).padLeft(2,0);
}
...
exports.findTrading = function(callback) {
    ...
    sync.fiber(function() {
        ...
        //sync.await(slackbotlib.sendMessage('Complete find trading', sync.defer()));
