...
exports.getBestStock = function(date, callback) {
    BestStock.findOne({'date': date}, callback);
};

exports.addBestStock = function(date, list, callback) {

    sync.fiber(function() {
        var result = sync.await(exports.getBestStock(date, sync.defer()));
        if(result === null) {
...
exports.filterBestStock = function(date, tradingList, callback) {
    sync.fiber(function() {

        var bestStockList = sync.await(exports.getBestStock(date, sync.defer()));
        if(!bestStockList) {
            throw 'best stock is null';
        }

        bestStockList = bestStockList.list;
        var result = [];
        for(var i = 0; i<tradingList.length; i++) {
            var trading = tradingList[i];
...
exports.findTop100Stocks = function(callback) {
    sync.fiber(function() {
        console.log('Find Top100 Stocks');
        var code = sync.await(otplib.requestTop100StocksOTP(sync.defer())).text;
        var today = moment();
        var body = {
            ind_tp: 'KSQ', //KSQ: KOSDAQ, STK: KOSPI
            idx_type: 2001, //KSQ: 2001, STK: 1001
            sect_tp_cd: null,
        ...
        sync.await(exports.addBestStock(today.format('YYYYMMDD'), result, sync.defer()));
